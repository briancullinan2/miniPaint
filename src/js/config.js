...
/**
 * Recursively deletes every property, empties arrays, and nullifies 
 * all references within an object to force garbage collection.
 * @param {Object|Array} obj - The target structure to completely destroy.
 */
function deepPurgeObject(config) {
	if (config === null || config === undefined || typeof config !== 'object') {
		return;
	}

	// Handle Objects: Traverse all properties (including hidden or Symbol properties)
	const keys = Reflect.ownKeys(config);
	for (const key of keys) {
		try {
			const value = config[key];

			// Delete the property link entirely from the engine
			// If it's a non-configurable property, fallback to nullifying it
			if (!delete config[key]) {
				config[key] = null;
			}
		} catch (e) {
			// Protect against sealed/frozen object property descriptors
			try { config[key] = null; } catch (_) { }
		}
	}


	config.deepPurgeObject = deepPurgeObject;
